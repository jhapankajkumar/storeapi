from flask_restful import Resource, reqparse
from flask_jwt import jwt_required
from models.item import ItemModel


class Item(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('price',
                        type=float,
                        required=True,
                        help="This filed can not be empty")

    parser.add_argument('store_id',
                        type=int,
                        required=True,
                        help="store id must not be empty")

    @jwt_required()
    def get(self, name):
        item: ItemModel = ItemModel.get_item_by_name(name)
        if item:
            return item.json()
        return {"message": "No record found"}, 404

    @jwt_required()
    def post(self, name):
        if ItemModel.get_item_by_name(name):
            return {"message": "Item already added"}, 400
        data = Item.parser.parse_args()

        item = ItemModel(name, **data)
        try:
            item.save_to_db()
        except:
            return {"message": "An error occurred inserting the item."}, 500

        return item.json(), 201

    @jwt_required()
    def delete(self, name):
        item = ItemModel.get_item_by_name(name)
        if item is None:
            return {"message": "No record found"}, 404
        item.delete_from_db()
        return {"message": "Item deleted"}

    @jwt_required()
    def put(self, name):
        data = Item.parser.parse_args()
        item:ItemModel = ItemModel.get_item_by_name(name)

        if item is None:
            item = ItemModel(item.name, data['price'], data['price'])
        else:
            item.price = data['price']
            item.store_id = data['store_id']

        item.save_to_db()
        return item.json(), 201


class ItemList(Resource):
    # @jwt_required
    def get(self):
        items = ItemModel.get_all_item()
        return {'items': items}
